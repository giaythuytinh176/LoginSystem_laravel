...
@section('content')
    ...
    <div id="logreg-forms">
        ...
        @if(\Illuminate\Support\Facades\Session::has("success"))
            <div>{{ \Illuminate\Support\Facades\Session::get("success") }}</div>
        @endif
            @if(\Illuminate\Support\Facades\Session::has("errorUP"))
                <p style="color: red">   <div>{{ \Illuminate\Support\Facades\Session::get("errorUP") }}</div></p>
            @endif
        {{--            {{ dd($errors) }}--}}
        @if($errors->any())
        ...
        @endif
            ...
            @if(@Auth::check())
                ...
                Ban da dang nhap roi nhe
                @endif

            <div><a class="" href="{{ route("logout") }}">Logout</a></div>
        <form method="post" action="{{ route("login.login") }}" class="form-signin"  enctype="multipart/form-data">
            ...
            <input type="email" name="email" id="inputEmail" value="{{ old("email") }}" class="form-control" placeholder="Email address" required="" autofocus="">
            <input type="password" name="password" id="inputPassword" value="{{ old("password") }}"  class="form-control" placeholder="Password" required="">
            <input type="hidden" name="test1" id="inputPassword" value="{{ old("email") }}"  class="form-control">
            <input type="hidden" name="test2" id="inputPassword" value="{{ old("password") }}"  class="form-control">
{{--            <input type="file" name="upload">--}}
        ...
        <form action="{{ route("login.store") }}" method="post" class="form-signup">
            <div class="social-login">
            ...
            <input type="text" name="username" class="form-control" placeholder="Full name" required="" autofocus="">
